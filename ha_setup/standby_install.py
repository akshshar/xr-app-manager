...
class StandbyInstall(ZtpHelpers):
    ...
    def __init__(self,
        ...
        super(StandbyInstall, self).__init__(syslog_file=syslog_file,
...
if __name__ == "__main__":
    ...
    standby_install = StandbyInstall()
    #Check if there is a standby RP on the system. If not, abort.

    standby_ip = standby_install.get_peer_rp_ip()

    if standby_ip["status"] == "error":
        standby_install.syslogger.info("No standby RP detected or failed to get standby RP xrnns ip. Aborting...")
        sys.exit(0)

    if results.input_files is None:
        standby_install.syslogger.info("No input file provided, checking for directories to sync...")
    else:
        standby_install = StandbyInstall()

        for filename in results.input_files:
            # Execute scp_to_standby for each file provided as input
            scp_output = standby_install.scp_to_standby(src_path=os.path.abspath(filename), 
                                           dest_path=os.path.abspath(filename), 
                                           preserve_perms=True)
            if scp_output["status"] == "error":
                standby_install.syslogger.info("Failed to set up file: "+str(os.path.abspath(filename))+" on the standby RP")
                sys.exit(1)
            else:
                standby_install.syslogger.info("Successfully set up file: "+str(os.path.abspath(filename))+" on the standby RP")



    if results.input_directories is None:
        standby_install.syslogger.info("No input directories provided...")
    else:
        for dirname in results.input_directories:
            # Execute scp_to_standby for each directory provided as input
            scp_output = standby_install.scp_to_standby(dir_sync=True,
                                           src_path=os.path.abspath(dirname),
                                           dest_path=os.path.abspath(dirname),
                                           preserve_perms=True)
            if scp_output["status"] == "error":
                standby_install.syslogger.info("Failed to set up directory: "+str(os.path.abspath(dirname))+" on the standby RP")
                sys.exit(1)
            else:                                 
                standby_install.syslogger.info("Successfully set up directory: "+str(os.path.abspath(dirname))+" on the standby RP")


    if results.standby_bash_cmds is None:
        standby_install.syslogger.info("No Standby RP bash commands provided...")
    else:
        for cmd in results.standby_bash_cmds:
            # Run execute_cmd_on_standby for each bash command provided as input
            standby_bash_cmd = standby_install.execute_cmd_on_standby(cmd = cmd)
            
            if standby_bash_cmd["status"] == "error":
                standby_install.syslogger.info("Failed to execute bash cmd: \""+str(cmd)+"\" on the standby RP. Output: "+str(standby_bash_cmd["output"])+". Error: "+str(standby_bash_cmd["error"]))
                sys.exit(1)
            else:
                standby_install.syslogger.info("Successfully executed bash cmd: \""+str(cmd)+"\" on the standby RP. Output: "+str(standby_bash_cmd["output"]))
             
    standby_install.syslogger.info("Done!")
    sys.exit(0)
